...
class MedicalDashboard(QMainWindow):
    def __init__(self):
        ...
        # State variables for flow rate fallback calculation
    ...
    def _header(self):
        ...
        t = QLabel("MEDICAL  PRESSURE  DASHBOARD")
    ...
    def toggle_connection(self):
        if not self.is_connected:
            ...
            self.live_data = []
            self._prev_volume = None  # Reset flow rate calculation state
            self._prev_time = None
    ...
    def _read_serial(self):
        ...
        try:
            # Drain the entire serial buffer to eliminate lag
            while self.serial_port.in_waiting > 0:
                ...
                # Parse: "Volume,FlowRate"
                parts = [p.strip() for p in raw.split(",")]

                # Extract volume (parts[0]) - required
                try:
                    volume = float(parts[0])
                except (ValueError, IndexError):
                    continue  # Skip malformed lines

                # Primary: Extract flow rate from Arduino (parts[1])
                flow_rate = None
                try:
                ...
                # Fallback: Calculate flow rate if not provided by Arduino
                if flow_rate is None:
                    if self._prev_volume is not None and self._prev_time is not None:
                        dt = current_time - self._prev_time
                        if dt > 0:
                            flow_rate = (volume - self._prev_volume) / dt
                        else:
                            flow_rate = 0.0
                    else:
                        flow_rate = 0.0  # First reading, no previous data

                # Store for logging/export (no underscore prefix for CSV export)
                row = {
                    "Timestamp": datetime.now().strftime("%H:%M:%S.%f")[:-3],
                    "Volume_ml": volume,
                    "FlowRate_ml_s": flow_rate,
                }
                self.live_data.append(row)

                # Update state for next fallback calculation
                self._prev_volume = volume
                self._prev_time = current_time

                # Keep track of the latest valid data
                latest_volume = volume
                latest_flow_rate = flow_rate
                latest_time = current_time

            # Only update UI once with the latest data after draining buffer
    ...
    def _update_ui(self, volume: float, flow_rate: float):
        ...
        # Display real-time flow rate (from Arduino or calculated fallback)
        self.lbl_min.setText(f"{flow_rate:.1f}")

        # Update graph with volume data
        w = self.live_data[-WINDOW_SIZE:]
        self.graph.update_plot(
            list(range(len(w))), [d["Volume_ml"] for d in w if "Volume_ml" in d]
        )
    ...
    def export_csv(self):
        ...
        # Ordered list of patient demographic keys (Arduino_m is intentionally excluded)
        demo_keys = [
            "Name",
            "Age",
            "Gender",
        ...
        try:
            with open(path, "w", newline="", encoding="utf-8") as f:
                ...
                # ── Row 1: patient demographic headers ──────────────────────
                writer.writerow(demo_keys)

                # ── Row 2: patient demographic values ───────────────────────
                writer.writerow([self.patient_info.get(k, "") for k in demo_keys])

                # ── Row 3: blank separator ───────────────────────────────────
                writer.writerow([])

                # ── Row 4: time-series column headers ───────────────────────
                writer.writerow(["Timestamp", "volume", "volumetric rate"])

                # ── Row 5+: time-series data rows ────────────────────────────
                for entry in self.live_data:
                    writer.writerow([
                        entry.get("Timestamp", ""),
                        entry.get("Volume_ml", ""),
                        entry.get("FlowRate_ml_s", ""),
                    ])
